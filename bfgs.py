...
def bfgs(x0, problem, options, search):
    ...
    for itr in range(options['max_iter']):
        ...
        if y_k @ s_k > eps * np.linalg.norm(y_k) * np.linalg.norm(s_k):
            hessian_k = bfgs_update(s_k, y_k, hessian_k)
...
def bfgs_update(s_k, y_k, H_k):
    """ BFGS equation for updating the approximate inverse Hessian
    ...
    rho_k = 1.0 / (y_k @ s_k)
    I = np.eye(len(s_k))
    H_kp1 = (I - rho_k * np.outer(s_k, y_k)) @ H_k @ (I - rho_k * np.outer(y_k, s_k)) 
    H_kp1 = H_kp1 + rho_k * np.outer(s_k, s_k) 
    return H_kp1
